...
def main(base_dir, num_epochs, viz):
    ...
    tf_section_cloud_files = glob.glob(os.path.join(base_dir, "tf_section_cloud_*.ply"))
    tf_section_cloud_files.sort()
    tf_section_cloud_files = tf_section_cloud_files[::2]
    ...
    for cloud_file, tf_file in section_cloud_files:
        ...
        for seam_pt in seam_pts:
            inds = pcd_tree.query_ball_point(seam_pt, 0.1)
            for ind in inds:
