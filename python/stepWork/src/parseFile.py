...
# https://github.com/tpaviot/pythonocc-core/blob/9e177ec503fcd6372fd28312137d999d553dedae/src/Extend/DataExchange.py
#####
import os
...
def stepUtils():
    stepfile = p21.readfile("../data/model.STEP")

    for entity in stepfile.header.entities:
        print(entity, stepfile.header.get(entity))


def write_stl_file(
    a_shape, filename, mode="ascii", linear_deflection=0.01, angular_deflection=0.5
):
...
def occ_readFile(filename):
    shapes = read_step_file_with_names_colors(filename)

    results_dir = f"../data/results/{filename.split('/')[-1].split('.')[0]}"
    if not os.path.exists(results_dir):
        os.makedirs(results_dir)
    for i,shape in enumerate(shapes):
        write_stl_file(shape, f"{results_dir}/shape_{i}.stl")

    for k,v in shapes.items():
        print(k,v)

    # my_renderer = threejs_renderer.ThreejsRenderer()#background_color="#123345")
    # my_renderer.DisplayShape(list(shapes.keys())[42])

    display, start_display, add_menu, add_function_to_menu = init_display()
    display.DisplayShape(list(shapes.keys()), update=True)
    # display.DisplayShape(list(shapes.keys()[42]), update=True)
    start_display()


# stepUtils()
# occ_helloWorld()
occ_readFile("../data/yoshi_hex/Yoshi_Hex.STEP")
